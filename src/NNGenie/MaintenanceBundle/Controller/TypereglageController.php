    ...
    public function newAction(Request $request) {
        ...
        if ($request->isMethod("POST") || $request->isMethod("GET")) {
            if ($form->isSubmitted() && $form->isValid()) {
                $typereglageUnique = $repositoryTypereglage->findBy(array("intitule" => $typereglage->getIntitule(), "statut" => 1));
                if ($typereglageUnique == null) {
                    try {
                        ...
                        $typereglage = new Typereglage();
                        $form = $this->createForm(new TypereglageType(), $typereglage);
                        return $this->render('NNGenieMaintenanceBundle:Typereglages:form-add-typereglage.html.twig', array('form' => $form->createView(), 'reglages' => $reglages));
                    } catch (Exception $ex) {
        ...
        if ($request->isMethod("POST") || $request->isMethod("GET")) {
            if ($editForm->isSubmitted() && $editForm->isValid()) {
                try {
                    ...
                    $this->get('ras_flash_alert.alert_reporter')->addError($message);
                    return $this->render('NNGenieMaintenanceBundle:Typereglages:form-update-typereglage.html.twig', array('form' => $editForm->createView(), 'idtypereglage' => $typereglage->getId(), 'reglages' => $othersProducts));
                }
            }
            foreach ($typereglage->getReglages() as $reglage) {
